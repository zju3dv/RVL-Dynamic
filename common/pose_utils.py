...
def np_qlog_t(q):
    n = np.linalg.norm(q[:, 1:], ord=2, keepdims=True, axis=1)
    n = np.clip(n, a_min=1e-8, a_max=1e8)
    q = q[:, 1:] * np.arccos(np.clip(q[:, :1], a_min=-1.0, a_max=1.0))
    q = q / n
    return q
...
def process_sensetime_poses(poses_in, mean_t, std_t, align_pose_R, align_pose_t, align_s):
    """
    processes the 1x12 raw pose from dataset by aligning and then normalizing
    :param poses_in: N x 12
    :param mean_t: 3
    ...
    R0 = align_pose_R.transpose()
    t0 = np.dot(-R0,align_pose_t[:, None]).squeeze()
    print(t0)

    # align
    for i in xrange(len(poses_out)):
        R = poses_in[i].reshape((3, 4))[:3, :3]
        q = txq.mat2quat(np.dot(R0, R))
        q *= np.sign(q[0])  # constrain to hemisphere
        q = qlog(q)
        poses_out[i, 3:] = q

        t = poses_out[i, :3]
        t = np.dot(R0, t[:, None]).squeeze() + t0
        poses_out[i, :3] = align_s * t
        # poses_out[i, :3] = align_s * np.dot(align_R, t[:, np.newaxis]).squeeze()

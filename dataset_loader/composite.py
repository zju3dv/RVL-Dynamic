...
class MF(data.Dataset):
    ...
    """
    def __init__(self, dataset, include_vos=False, no_duplicates=False, unsupervise=False,
                 *args, **kwargs):
        """
        ...
        if dataset == '7Scenes':
            ...
            self.dset = RobotCar(*args, real=self.real, unsupervise=unsupervise, **kwargs)
            if self.include_vos and self.real:
                self.gt_dset = RobotCar(*args, skip_images=True, real=False, unsupervise=unsupervise,
                                        **kwargs)

        elif dataset == 'SenseTime':
            from sensetime import SenseTime
            self.dset = SenseTime(*args, **kwargs)
        elif dataset == 'Cambridge' or dataset == 'NewCambridge':
    ...
    def get_indices(self, index):
        ...
        offsets = np.insert(skips, 0, 0).cumsum() # (0, 10, 10) -> (0, 10, 20)
        offsets -= offsets[len(offsets) / 2] # (-10, 0, 10)
        if self.no_duplicates:
            offsets += self.steps/2 * self.skip
        offsets = offsets.astype(np.int)
        idx = index + offsets               # (50, 60, 70)
        idx = np.minimum(np.maximum(idx, 0), len(self.dset)-1) #(0 ~ dset)
...
class MaskMF(MF):
    def __init__(self, dataset, include_vos=False, no_duplicates=False, unsupervise=False,
        ...
        from maskrobotcar import RobotCar
        self.dset = RobotCar(*args, real=self.real, unsupervise=unsupervise, **kwargs)
        if self.include_vos and self.real:
            self.gt_dset = RobotCar(*args, skip_images=True, real=False, unsupervise=unsupervise,
                                    **kwargs)
    ...
    def __getitem__(self, index):
        ...
        imgs  = torch.stack([c[0][0] for c in clip], dim=0)
        masks = torch.stack([c[0][1] for c in clip], dim=0)
        poses = torch.stack([c[1] for c in clip], dim=0)

        return (imgs, masks), poses
...
class MFOnline(data.Dataset):
    ...
    def __init__(self, gps_mode=False, *args, **kwargs):
        ...
        self.train_set = MF(train=True, unsupervie=True, *args, **kwargs)
...
def main():
    ...
    import os.path as osp
    from maskrobotcar import RobotCar
    from dataset_loader.dataloaders import get_mask_transforms, get_train_transforms
    from common.utils import get_configuration
    from mapnet.config import MapNetConfigurator
    from dataloaders import get_mapnet_train_dataloader
    config = get_configuration(MapNetConfigurator())
    data_loader, val_dataloader = get_mapnet_train_dataloader(config)
    # scene = 'full'
    # num_workers = 4
    ...
    # transform, t = get_train_transforms(config)

    # data_path = osp.join('/home/drinkingcoder/Dataset/robotcar')
    # data_dir = '../data/RobotCar/full/'
    # dset = RobotCar(scene, data_path, train=True, real=True, mask_transform=mask_transform, transform=transform, data_dir=data_dir)
    ...
    for batch in data_loader:
        ...
        img = mask[0][0]
        print(img.size())
        print(torch.mean(mask[0][0]))
        img = transforms.ToPILImage()(img)
        # plt.imshow(batch[0][0, 3:])
        img.show()
        # show_batch(make_grid(img, nrow=5, padding=25, normalize=True))
        # img = batch[0][1][0] * 255
        # img = transforms.ToPILImage()(img)
        # plt.imshow(batch[0][0, 3:])
        # img.show()
